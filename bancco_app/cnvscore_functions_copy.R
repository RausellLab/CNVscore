...
matching_length <- function(bin_length = 100, tbl_input) {
  ...
  # bin_length <- 100
  # tbl_input <- input_check_cnv_del_pathogenic_clinvar %>%
...
predict_rtemis <- function(x, chrom_tmp, vector_chrom, df_predict, 
  ...
  # chrom_tmp <- 16
  # vector_chrom <- 3
  # df_predict <- output_clinvar_deletion
...
predict_chrom_aware_rtemis <- function(list_models, test_split,
  ...
  # test_split <- output_clinvar_deletion
  # 
  # tag_variant <- 'deletion'
...
predict_chrom_aware <- function(list_models, df_to_predict, only_table = FALSE,
  ...
  # list_models <- bancco_logistic_clinvar_del_length
  # df_to_predict <- output_clinvar_deletion
...
check_cnv_v2 <- function(input_df, mode_reg = FALSE, factor_clinical = TRUE) {
  ...
  read_competitors <- function(label_title,x, list_models) {
    ...
    return(list(result_main_table, roc_curves_table, pr_curves_table))
  ...
  get_results <- function(input_df, type_variant = 'DEL', tag_title, do_all = TRUE) {
    ...
    if (type_variant == 'DEL') {
      ...
      final_results_x_del <- read_competitors(label_title = 
      ...
      final_results_x_dup <- read_competitors(
  ...
  features_enrichment <- function(df_features, training_del, training_dup) {
    ...
    # df_features <- features_tbl[features_tbl$human_control == 'yes',]
    ...
    features_continuous <- df_features %>% filter(type == 'numerical') %>% pull(name)
    
    # !!!!!!!!! EXCLUDE TEMPORALLY ESSENT_XX FEATURES
    
    # features_categorical <- features_categorical[str_detect(features_categorical, 'essent', negate = TRUE)]
    ...
    p_enrich1 <- def_results_del %>% mutate(variant_type = 'Deletion') %>% 
      ...
      mutate(sig = if_else(p.value <= (0.05 / length(features_categorical)), 'yes', 'no')) %>%
      mutate(sig = factor(sig, levels = c('yes', 'no'))) %>%
      mutate(across(where(is.double), log10)) %>%
      ggplot(aes(estimate, reorder(feature, estimate))) +
      # geom_point(aes(fill = sig), shape = 21, color = 'black') +
      geom_pointrange(aes(xmin=conf.low, xmax=conf.high, color = sig), size = 0.8, show.legend = FALSE) +
      geom_vline(xintercept = 0, linetype = 'dashed') +
      theme_minimal() +
      scale_color_material_d() +
      facet_wrap(vars(variant_type)) +
      labs(title = 'Enrichment of categorical features (Pathogenic vs. Non-pathogenic CNVs)', y = 'Categorical features',
           subtitle = 'Bonferroni correction: statistically significant (blue) - no statistically significant (red)',
           x = 'Enrichment - log10(odds ratio)', color = 'Significant after Bonferroni correction')
    ...
    p_enrich2 <- def_results_cont_del %>% 
      ...
      ggplot(aes(reorder(feature, p.value), p.value)) + 
      geom_col(aes(fill = p.value), color = 'black', show.legend = FALSE) + 
      ...
      labs(y = '-log10(p.value)', fill = '-log10(p.value)', title = 'Enrichment of continuous features (Pathogenic vs. Non-pathogenic CNVs)',
           x = 'Continuous features', subtitle = 'Wilcoxon test - p-values equals to 0, a value of 1e-300 was applied \nVertical lines represent Significant treshold based on the Bonferroni test') +
      theme_minimal()
